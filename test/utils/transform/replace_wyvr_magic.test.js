...
describe('utils/transform/replace_wyvr_magic', () => {
    ...
    it('as client', () => {
        strictEqual(
            replace_wyvr_magic(test_content, CodeContext.client),
    ...
    it('as server', () => {
        strictEqual(
            replace_wyvr_magic(test_content, CodeContext.server),
            `<script> const a = false; const b = true; const a__var = [];</script><p>{false} thisisClient isclient {true} thisisServer isserver {__('translate')}</p>`
        );
    });
    it('injectConfig without fallback and value not found', () => {
        strictEqual(replace_wyvr_magic(`<script>const a = injectConfig('a')</script>`, CodeContext.server), '<script>const a = undefined</script>');
    });
    it('injectConfig without fallback and value found', () => {
        Config.replace({ a: true });
        strictEqual(replace_wyvr_magic(`<script>const a = injectConfig('a')</script>`, CodeContext.server), '<script>const a = true</script>');
    });
    it('injectConfig with fallback and value not found', () => {
        strictEqual(replace_wyvr_magic(`<script>const a = injectConfig('a', false)</script>`, CodeContext.server), '<script>const a = false</script>');
    });
    it('injectConfig with fallback and value found', () => {
        Config.replace({ a: true });
        strictEqual(replace_wyvr_magic(`<script>const a = injectConfig('a', false)</script>`, CodeContext.server), '<script>const a = true</script>');
    });
    ...
    it('injectConfig multiple times', () => {
        Config.replace({ a: true });
        strictEqual(replace_wyvr_magic(`<script>const a = injectConfig('a', false) || injectConfig('b', false)</script>`, CodeContext.server), '<script>const a = true || false</script>');
    });
    it('replace imports', () => {
        Config.replace({ a: true });
        strictEqual(
    ...
    it('replace imports as client', () => {
        Config.replace({ a: true });
        strictEqual(
            replace_wyvr_magic(`<script>import a from "gen/src/file"; import b from "somewhere";</script>`, CodeContext.client),
