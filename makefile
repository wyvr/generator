...
WYVR_COVERAGE=npx c8 -x src/action -x src/command -x src/command.js -x src/worker.js -x src/no_worker.js -x src/worker_action -x src/resource -x src/templates -x src/boilerplate --skip-full --clean $(WYVR_TEST)
