...
<nav>
    ...
    <button id="wyvr_debug_measure_lcp" title="Measure LCP">🖌</button>
</nav>
`;
...
const wyvr_debug_measure_lcp = document.getElementById('wyvr_debug_measure_lcp');
if (wyvr_debug_measure_lcp) {
    wyvr_debug_measure_lcp.addEventListener('click', (e) => {
        location = `${location.origin}${location.pathname}?wyvr_debug_measure_lcp`;
    });
}

if (window.location.search.indexOf('wyvr_debug_measure_lcp') > -1) {
    let lastLcp;
