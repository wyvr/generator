...
describe('utils/cron/filter_cronjobs', () => {
    ...
    it('returns empty array for empty input', () => {
        ...
        deepStrictEqual(log, [
            ['⚠', 'cron "a" missing when or what properties'],
        ]);
    });
    it('invalid when', () => {
        deepStrictEqual(
            filter_cronjobs({ a: { when: 'test', what: 'a' } }),
            []
        );
        deepStrictEqual(log, [
            ['⚠', 'cron "a" has an invalid when property "test"'],
        ]);
    });
    ...
    it('returns only jobs that should be executed now', () => {
        ...
        const expectedJobs = [
            { name: 'e', when: '* * * * *', what: 'e', failed: false },
        ];
    ...
    it('match the current time', () => {
        ...
        deepStrictEqual(filter_cronjobs({ a: { when, what: 'a' } }), [
            { name: 'a', when, what: 'a', failed: false },
        ]);
        deepStrictEqual(log, []);
    });
    it('ignore trigger based cronjobs', () => {
        const date = new Date();
        deepStrictEqual(
            filter_cronjobs({ a: { when: '@build', what: 'a' } }),
            []
        );
        deepStrictEqual(log, []);
    });
